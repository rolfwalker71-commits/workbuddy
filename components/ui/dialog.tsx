  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          // Grid items default to min-width:auto, so unbreakable content (e.g. the
          // "____" separator in a Teams invite) widens the track past max-width and
          // stops flex rows inside from wrapping. [&>*]:min-w-0 lets them shrink.
          "fixed top-1/2 left-1/2 z-[1100] grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 overflow-x-hidden rounded-xl bg-popover p-4 text-sm break-words text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none [&>*]:min-w-0 sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
          className
        )}
        {...props}
      >
