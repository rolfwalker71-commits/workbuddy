...
test("inferSystemsAffected reads categories and keywords", () => {
  ...
  assert.equal(eventMayAffectInternal(systems), true);
  assert.equal(eventMayAffectInternal(["ERP"]), false);
});
...
test("mapTechUpgradeEvent maps a Graph row without writing presence fields", () => {
  ...
  assert.equal(mapped?.mayAffectInternal, true);
