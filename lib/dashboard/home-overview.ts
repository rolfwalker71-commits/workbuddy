...
export async function refreshHomeKpis(auth: AuthContext): Promise<HomeKpiLive> {
  ...
  const [msLive, weatherLive] = await Promise.all([
  ...
  if (userId != null) {
    writeHomeUnreadCache(userId, { microsoftUnread });
  }
  if (weatherLive) writeHomeWeatherCache(userId, weatherLive);

  return { microsoftUnread, weather };
}
...
export async function getHomeDetails(
  ...
  const [todayEvents, msMail, tasks, lastTeams] = await Promise.all([
    withTimeout(
      loadWorkspaceTodayEvents(userId, { wantMicrosoft: showMs }),
    ...
    withTimeout(
      ...
      HOME_PROVIDER_TIMEOUT_MS,
      emptyTasks({ microsoftConnected: msConnected })
    ),
    msConnected &&
  ...
  return {
    microsoft: showMs
      ? {
          events: eventsFromToday(linkedEvents),
          mailInbox: msMail,
          tasks,
          lastTeams,
        }
